...
export const BaseScene: React.FC = () => {
  ...
  return (
    <Scene vo="vo_base.wav">
      <Sequence from={0} durationInFrames={SWAP + 5} layout="none">
        <Screen src="shot_base.mp4" zoomFrom={1.02} zoomTo={1.06} origin="50% 60%">
          <Callout title="User-signed decision receipt" body="Groundhog never holds the key. The wallet signs, the registry stores the decision." enter={70} exit={205} x={1180} y={850} width={560} />
        </Screen>
      </Sequence>
      <AbsoluteFill style={{ opacity: stillIn }}>
        <Sequence from={SWAP - 3} layout="none">
          <Screen src="explorer_crop.png" still zoomFrom={1.0} zoomTo={1.04} origin="30% 30%">
            ...
            <Callout title="Verified on-chain" body="recordDecision · allocation 500 bps · memoryApplied = true · policy and incident hashes stored." enter={22} exit={146} x={1140} y={700} width={640} />
          </Screen>
        </Sequence>
      </AbsoluteFill>
    </Scene>
